...
- 2026-02-17: studio agent revert — added an opt-in integration test that asserts OpenCode `revert` + `unrevert` actually change files in a temp repo, plus production diagnostics (diff summary + warnings for no-op reverts) and bucket-sync pausing during agent runs to reduce state corruption.
- 2026-02-17: studio bucket sync reliability — added an opt-in integration test that validates workspace + OpenCode state survive a studio shutdown/hydrate cycle using the production-like Studio image; prioritized syncing OpenCode state (snapshot/storage) and made the sync loop more responsive after pause windows (reduces missed syncs after agent actions).
- 2026-02-17: test runner — root `npm test` now runs backend unit + integration suites (integration is opt-in via `VIVD_RUN_INTEGRATION_TESTS=1`, plus per-test env flags).
