...
<Aside type="note" title="Install-wide email, not project form config">
  This surface does not replace project-level Contact Form setup. Recipient lists and
  form fields still belong in the project plugin settings.
</Aside>
...
The Contact Form plugin depends on email delivery, but it is not the same configuration
surface.
