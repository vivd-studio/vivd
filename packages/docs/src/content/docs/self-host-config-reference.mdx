...
```env
# auto   = use proxy/CDN country headers when present, else try local GeoLite2 lookup
# headers = only use proxy/CDN country headers
# maxmind = only use the local GeoLite2 database
# off     = disable country detection
...
GeoLite2 Country `.mmdb` file into the backend container at the configured path.

Vivd now includes a helper script for the common self-host case:
...
The script downloads `GeoLite2-Country.mmdb` into the default `./geoip/`
directory that the published Compose bundle already mounts into the backend.
