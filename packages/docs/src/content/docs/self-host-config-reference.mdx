...
Optional residential proxy for the bundled scraper's Puppeteer browser:
...
Set `PROXY_HOST` to enable it. Credentials are only used when both username and
password are present. This applies to the bundled `scraper` service in the published
Compose bundle; if you run a separate external scraper via `SCRAPER_URL`, configure
