...
<DocShot
  ...
  caption="Organization member management is where admins add people, change roles, and review who can access the workspace."
/>
...
4. Re-check membership before launch, project handoff, or staffing changes.

<Aside type="note" title="About older restricted roles">
  Some installs may still show an older project-restricted role from an earlier access model. Prefer `Owner`, `Admin`, and `User` for new assignments unless you are deliberately maintaining legacy behavior.
</Aside>
