...
{showOperatorGuides ? (
  <>
    <h2>Typical self-host defaults</h2>

    <p>
      On <code>solo</code> self-host installs, the main public host from{" "}
      <code>Instance Settings -&gt; Network</code> is always a publish target for
      the local instance. Solo installs can also use additional registered
      custom publish domains, but tenant-host style routing stays off.
    </p>
  </>
) : null}
...
{showOperatorGuides ? (
  <p>
    Read <a href="/instance-settings/">Instance Settings</a> for install-wide
    routing and capability settings.
  </p>
) : null}
