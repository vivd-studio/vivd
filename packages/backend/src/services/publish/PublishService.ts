...
export class PublishService {
  ...
  private getResolvedPrimaryHost(): string | null {
    const publicHost = instanceNetworkSettingsService.getResolvedSettings().publicHost;
    if (!publicHost) return null;
    return domainService.normalizeDomain(publicHost);
  }

  private isPrimaryHostDomain(domain: string): boolean {
    const primaryHost = this.getResolvedPrimaryHost();
...
${redirectRulesBlock}
    # Matcher to exclude Vivd runtime routes on the primary host
    @primaryPublishedNotVivdRuntime not path /vivd-studio /vivd-studio/* /_studio /_studio/* /plugins /plugins/* /email/v1/feedback /email/v1/feedback/* /_vivd_s3 /_vivd_s3/* /health

    # Serve the published site from the primary host without creating a second site block
    handle @primaryPublishedNotVivdRuntime {
        root * ${publishedPath}
...
`;
      ...
      fs.writeFileSync(this.getPrimaryHostInlineConfigPath(), primaryConfig, "utf-8");
      return;
    }
...
`;
  ...
  private removeCaddyConfig(domain: string): void {
    ...
    if (this.isPrimaryHostDomain(domain)) {
      const primaryConfigPath = this.getPrimaryHostInlineConfigPath();
      if (fs.existsSync(primaryConfigPath)) {
        fs.unlinkSync(primaryConfigPath);
      }
    }
  }
