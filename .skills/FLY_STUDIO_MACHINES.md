...
Use this note when touching Fly Studio machine startup, wake/reconcile behavior, connected-mode auth, or when debugging slow/broken Studio boots in prod.

### When To Read This

- A Studio machine is waking slowly or cold-booting unexpectedly.
- A reconciled/suspended machine does not resume fast.
- A connected Studio reports backend/auth/usage failures.
- You are changing machine env, reconcile logic, runtime auth, or release/smoke coverage for Fly Studios.
...
- If Studio shows misleading “usage limit” style failures, inspect runtime callback logs first.
  - We saw real prod failures where the root cause was `401 Studio runtime is not authorized for this organization`, not limit exhaustion.

### Fly Debugging Tips

- You can use the local user's `fly` or `flyctl` CLI directly when available.
- You can also use SSH access to the prod backend host to fetch env-backed Fly credentials or inspect containers if the user explicitly provides that access in the conversation.
- If SSH access would help and the user has not provided the host or access details yet, ask for them explicitly instead of guessing.
- Useful things to inspect:
  - `fly logs -a vivd-studio-prod -i <machine-id>`
  - machine events/state/image/env
...
- Local release flow starts with `npm run publish:tag`, which runs `scripts/publish.sh`.
- Tag-triggered GitHub workflows then run shared validation and image smoke tests.
- If you touch Fly Studio lifecycle/auth behavior, proactively suggest adding the right regression or smoke to that pipeline instead of leaving it as tribal knowledge.
