...
- If the prod-shaped smoke still looks harsher than production, reduce the remaining lifecycle skew before changing suspend behavior again. As of April 17, 2026, the workflow can pin that smoke to one dedicated validate-app project slug and reuse the same machine across runs so the signal comes from a long-lived machine with history, which is closer to real production reconcile than a brand-new machine per release.
...
- Before changing reconcile/suspend logic again, make sure the prod-shaped smoke failure already includes the current machine evidence. `fly_prod_shape_reconcile_wake_auth.test.ts` should report the final machine snapshot, machine-list summary/URLs, best-effort runtime `cleanup/status`, recent events, and tail Fly logs on reconcile-park failure; use that data to decide whether the mismatch is in cleanup, lifecycle timing, or the validate environment itself.
