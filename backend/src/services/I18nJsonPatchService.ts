 ...
 */
function findLocaleFile(versionDir: string, lang: string): string | null {
  for (const localeDir of LOCALE_PATHS) {
    const filePath = path.join(versionDir, localeDir, `${lang}.json`);
 ...
 */
function getDefaultLocalePath(versionDir: string, lang: string): string {
  // Check if any locale directory already exists
  for (const localeDir of LOCALE_PATHS) {
    const dirPath = path.join(versionDir, localeDir);
    if (fs.existsSync(dirPath) && fs.statSync(dirPath).isDirectory()) {
      return path.join(dirPath, `${lang}.json`);
    }
  }
  // Default to locales/ in project root
  return path.join(versionDir, "locales", `${lang}.json`);
}
 ...
 *
 * @param versionDir - Absolute path to the project version directory
...
export function applyI18nJsonPatches(
  ...
  for (const [lang, langPatches] of patchesByLang.entries()) {
    ...
    let localePath = findLocaleFile(versionDir, lang);
    const isNewFile = !localePath;

    if (!localePath) {
      localePath = getDefaultLocalePath(versionDir, lang);
    }
...
export function extractI18nPatches(
  ...
  return patches
    .filter(
      (p): p is { type: "setI18n"; key: string; lang: string; value: string } =>
        p.type === "setI18n"
    )
