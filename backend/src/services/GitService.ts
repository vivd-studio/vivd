...
export class GitService {
  ...
  private buildGitHubRepoName(slug: string, version: number): string {
  ...
  async syncPushToGitHub(params: {
    ...
    const repo = this.buildGitHubRepoName(params.slug, params.version);
    ...
    try {
      ...
      await gitHubApiService.ensureOrgRepoExists(settings.org, repo, settings);
      ...
      return { attempted: true, success: true, repo, remoteUrl };
    } catch (error) {
      ...
      return { attempted: true, success: false, repo, remoteUrl, error: msg };
    }
  }
  ...
  async syncPullFromGitHub(params: {
    ...
    try {
      ...
      // Avoid auto-merges/rebases on preview open (they can create conflicts and leave the repo in a broken state).
      // We only fast-forward. If histories diverged, we skip and let the user decide how to reconcile.
      try {
        await this.gitWithHttpAuth(params.cwd, settings.token, [
          "fetch",
          settings.remoteName,
      ...
      const { stdout: counts } = await execa(
        "git",
        ["rev-list", "--left-right", "--count", `HEAD...${settings.remoteName}/main`],
        { cwd: params.cwd }
      );
      ...
      if (!Number.isFinite(ahead) || !Number.isFinite(behind)) {
        return {
          ...
          repo,
          remoteUrl,
          error: `Failed to parse rev-list counts: "${counts.trim()}"`,
        };
      }
      ...
      if (ahead > 0) {
        return {
          ...
          repo,
          remoteUrl,
          skippedReason: "Local and remote diverged",
        };
      }

      await execa(
        "git",
        ["merge", "--ff-only", `${settings.remoteName}/main`],
        { cwd: params.cwd }
      );
