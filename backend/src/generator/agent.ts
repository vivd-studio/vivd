    ...
    while (attempts <= maxRetries) {
      ...
      try {
        ...
        if (html) {
          ...
          log(
            `Warning: No HTML content extracted from response on attempt ${attempts}`,
          );
          ...
          if (attempts > maxRetries) {
            log(
              "Error: No content generated from OpenRouter after all retries",
            );
          }
        }
      } catch (error) {
