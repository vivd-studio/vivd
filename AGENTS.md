...
- Plugin packages should now export a safe package descriptor (for example `packages/plugin-analytics/src/descriptor.ts`) that carries plugin-owned definition/shared-UI/CLI metadata, while host workspaces build their own backend/frontend/CLI descriptor lists from those package exports. Keep backend runtime adapters host-owned, but derive as much registration metadata as possible from the package descriptor rather than maintaining separate hardcoded arrays/maps.
